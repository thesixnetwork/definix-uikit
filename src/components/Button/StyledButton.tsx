...
const StyledButton = styled.button<BaseButtonProps>`
  ...
  &:hover:not(:disabled):not(.definix-button--disabled):not(.definix-button--disabled):not(:active) {
    &:after {
      content: "";
      border-radius: 8px;
      background-color: ${({ theme, variant }) => variant === ButtonVariants.LINE ? `rgba(0, 0, 0, 0.05)` : hexToRGB(theme.colors[ColorStyles.WHITE], 0.2)};
      position: absolute;
      width: 100%;
      height: 100%;
      left: 0;
  ...
  &.definix-button--disabled:not(.definix-button--loading) {
    background-color: ${({ theme, variant }) => hexToRGB(theme.colors[ColorStyles.LIGHTGREY], variant === ButtonVariants.LINE ? 0.2 : 1)};
    color: ${({ theme, variant }) => variant === ButtonVariants.LINE ? hexToRGB(theme.colors[ColorStyles.MEDIUMGREY], 0.4) : hexToRGB(theme.colors[ColorStyles.WHITE], 0.5)};
    cursor: not-allowed;
  }
